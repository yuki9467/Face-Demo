...
public class Test {
	...
	public static void main(String[] args) {
		...
		HttpRequests httpRequests = new HttpRequests("hsNW9B1HLUBevgkCvxLioj_IPqBMMEwP", "O1GJnOGsCVTlsQ2i4_IeQJpm4dq4KDPF ", true, true);
		...
		try {
			...
			//person/add_face 通过personAddFace添加人脸
			System.out.println("\nperson/add_face");
			for (int i = 0; i < result.getJSONArray("face").length(); ++i)
			...
			//person/set_info 通过setPersonName添加中文姓名
			System.out.println("\nperson/set_info");
			for (int i = 0; i < result.getJSONArray("face").length(); ++i) {
				new PostParameters().setPersonName("person_"+i).setTag("涓枃 tag_"+i).getMultiPart().writeTo(System.out);
				System.out.println(httpRequests.personSetInfo(new PostParameters().setPersonName("person_"+i).setTag("涓枃 tag_"+i)));
			}
			
			//person/get_info 最后通过personGetInfo得到每一个Person的信息输出
			System.out.println("\nperson/get_info");
			for (int i = 0; i < result.getJSONArray("face").length(); ++i)
				System.out.println(httpRequests.personGetInfo(new PostParameters().setPersonName("person_"+i)));
			
			//-----------------Faceset-----------------创建Faceset，此处和创建Person基本一致
			//faceset/create 
			System.out.println("\nfaceset/create");
			for (int i = 0; i < result.getJSONArray("face").length(); ++i)
			...
			System.out.println("\nfaceset/set_info");
			for (int i = 0; i < result.getJSONArray("face").length(); ++i) {
				new PostParameters().setFacesetName("faceset_"+i).setTag("涓枃 tag_"+i).getMultiPart().writeTo(System.out);
				System.out.println(httpRequests.facesetSetInfo(new PostParameters().setFacesetName("faceset_"+i).setTag("涓枃 tag_"+i)));
			}
			...
			//-----------------Group-----------------创建一个Group，把所有Person加入到Group中去
			//group/create
			System.out.println("\ngroup/create");
			...
			//-----------------Recognition-----------------通过trainIdentify建立人脸标识模型，再通过trainVerify建立人脸验证模型
			...
			//recognition/recognize 调用recognitionIdentify标识了一张孙燕姿的照片，console中将输出识别成功，但confidence只有10%
			...
			//recognition/verify 调用recognitionVerify验证了两张人脸图片，一张返回True一张返回False
			System.out.println("\nrecognition/verify");
			System.out.println(
